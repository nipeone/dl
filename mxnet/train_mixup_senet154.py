...
parser.add_argument('--model', default="senet154", type=str,
                    help='model type (default: senet154)')
...
filehandler = logging.FileHandler('log/train_mixup_senet154.log')
...
logger.info(args)
if args.mixup:
    print ("use mixup method to train.")
...
def train(net,train_data,epochs,ctx):
    ...
    for epoch in range(epochs):
        ...
        for _, batch in enumerate(train_data):
            ...
            with ag.record():
                outputs = [net(X.astype(np.float32,copy=False)) for X in data]
                losses = [L(yhat, y.astype(np.float32,copy=False)) for yhat, y in zip(outputs, label)]
            for l in losses:
                l.backward()
...
def checkpoint(net,epoch):
    net.export(os.path.join(ckpt_path,'pdl-mixup-senet'),epoch=epoch)
